...
### Icon
**Use:** `dui-icon` with `lucide-static` imports (recommended) — never raw SVG with manual `width`/`height`
```ts
import { unsafeHTML } from "lit/directives/unsafe-html.js";
import { Home, Search } from "lucide-static";

// In your Lit template:
html`<dui-icon>${unsafeHTML(Home)}</dui-icon>`
html`<dui-icon style="--icon-size: 1.25rem; --icon-color: var(--text-2);">${unsafeHTML(Search)}</dui-icon>`
```
**Fallback:** Copy-paste SVG directly into the `<dui-icon>` slot when using a non-Lucide icon set:
```html
<dui-icon><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" ...>...</svg></dui-icon>
```
