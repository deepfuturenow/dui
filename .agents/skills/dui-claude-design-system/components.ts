...
{ tag: "dui-icon", group: "Data Display", keywords: ["icon", "svg", "symbol", "glyph", "lucide"],
  ...
  notes: `Source icons from **Lucide** (https://lucide.dev) — the full ~1,600-icon set. Names are kebab-case (\`arrow-right\`, \`chevron-down\`, \`circle-check\`); browse them at lucide.dev/icons and use exact names. For common icons, inline the SVG from memory (paths are stable); for less-common ones or exact parity, fetch the SVG from \`https://unpkg.com/lucide-static/icons/<name>.svg\` rather than guessing. Inline it into the default slot with \`fill="none"\`, \`stroke="currentColor"\`, round caps/joins. Size via \`--icon-size\`, color via \`--icon-color\` (defaults to \`currentColor\`). Tune \`stroke-width\` to size: \`2\` at the default ~20–24px, ~\`2.25\`–\`2.5\` for small (≤16px), ~\`1.5\` for large (≥32px). Composes inside other components too — a leading icon in \`<dui-button>\`, \`<dui-toggle slot="icon">\`, menu items, etc.`,
