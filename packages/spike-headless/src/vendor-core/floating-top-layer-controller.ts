/**
 * COPIED from `@dui/core/floating-top-layer-controller` for this spike, with one
 * added option, marked SPIKE CHANGE. `dui-primitives` is not modified.
 *
 * Note what did NOT need changing: this controller already takes `getAnchor()`
 * and `getPopover()` callbacks and renders nothing, so handing it refs from a
 * consumer's template instead of the primitive's `querySelector` results is a
 * drop-in. See FINDINGS question 2.
 */
/**
...
export type FloatingTopLayerControllerOptions = {
  ...
  /**
   * SPIKE CHANGE. Returns the element that actually scrolls the popup, for
    ...
    this.#cleanupAutoUpdate = startFixedAutoUpdate(anchor, popover, {
      ...
      alignToInner: this.#opts.alignToInner
        ? {
          ...
          getScrollContainer: this.#opts.getScrollContainer, // SPIKE CHANGE
        }
        : undefined,
