/**
 * COPIED from @dui/core/floating-popup-utils via Experiment 2's vendored copy,
 * with the P6 fixes applied in full:
 *
 *   1. resolveScrollContainer() is GONE. It found the popup's scroller by the
 *      class name `.Popup`; for consumer-named markup it returned null and
 *      inner alignment positioned the select popup 353px off with nothing
 *      logged. Both call sites now require the injectable getScrollContainer.
 *   2. renderArrow() is gone: core renders nothing in BUI.
 *   3. No lit import remains.
 */
/**
...
export type AlignInnerOptions = {
  ...
  padding?: number;
  /**
   * Returns the element that actually scrolls the popup. Required (P6): core
   * never finds it by class name.
   */
