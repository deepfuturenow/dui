/**
 * COPIED from @dui/core/floating-top-layer-controller via Experiment 2, with
 * the P6 getScrollContainer option threaded through. Already headless: takes
 * getAnchor()/getPopover() callbacks and renders nothing.
 *
 * Note the ref-timing hazard Experiment 2 found: a `ref` callback fires when
 * the element is CREATED, which for a custom element is before its own
 * internals exist. That is why these are getter callbacks resolved lazily on
 * every open/position, never captured eagerly.
 */
/**
...
/**
 * P6/D6: framework-free. The old copy required a Lit ReactiveControllerHost
 * and registered itself with host.addController(); that dragged lit types into
 * every behavior controller that positions a popup. Now it takes anything with
 * requestUpdate() and exposes dispose() for the owner's disconnect hook.
 */
type MinimalHost = { requestUpdate(): void };
...
export type FloatingTopLayerControllerOptions = {
  ...
  /**
   * P6. Returns the element that actually scrolls the popup, for
   * inner alignment. Has to be threaded through here as well as
   * `AlignInnerOptions`, because this controller builds that options object
   * itself from its own flattened `alignToInner*` fields.
   */
...
export class FloatingTopLayerController {
  ...
  /** Call from the owner's disconnected lifecycle. */
  dispose(): void {
    this.#stopAutoUpdate();
  }
    ...
    this.#cleanupAutoUpdate = startFixedAutoUpdate(anchor, popover, {
      ...
      alignToInner: this.#opts.alignToInner
        ? {
          ...
          getScrollContainer: this.#opts.getScrollContainer ?? (() => null), // P6
        }
        : undefined,
